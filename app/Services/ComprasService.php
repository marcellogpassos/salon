...
class ComprasService implements ComprasServiceInterface {
	...
	public function buscar($criterios) {
		$where = [];
		if (filtroFornecido($criterios, 'data_inicial'))
			array_push($where, ['data_compra', '>=', $criterios['data_inicial']]);
		if (filtroFornecido($criterios, 'data_final'))
			array_push($where, ['data_compra', '<=', $criterios['data_final']]);
		if (filtroFornecido($criterios, 'valor_minimo'))
			array_push($where, ['valor_total', '>=', $criterios['valor_minimo']]);
		if (filtroFornecido($criterios, 'valor_maximo'))
			array_push($where, ['valor_total', '<=', $criterios['valor_maximo']]);
		if (filtroFornecido($criterios, 'cliente'))
			array_push($where, ['cliente_id', $criterios['cliente']]);
		return $this->comprasRepository->findWhere($where);
	}
